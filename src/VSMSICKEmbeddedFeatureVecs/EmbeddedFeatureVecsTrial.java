...
public class EmbeddedFeatureVecsTrial {
	...
	public static void main(String... args) throws ClassNotFoundException {
		...
		LOGGER = VSMLogger.setup(EmbeddedFeatureVecsTrial.class.getName() + "."
				+ nonTerminal);
		...
		sentenceDirectories = VSMUtil
				.getSICKSentenceDirectories(VSMContant.SICK_TRIAL_SENTENCES_WITH_CHUNKS);
		...
		if (sentenceDirectories == null) {
			LOGGER.log(
					...
					"Sentence Directories Cannot be Fetched Something Wrong with the File system again: Path Trying to read: "
							+ VSMContant.SICK_TRIAL_SENTENCES_WITH_CHUNKS);
			System.exit(-1);
		}
				...
				if (chunkDirectory != null) {
					...
					if (chunkBinaryVectors != null) {
						...
						for (File vector : chunkBinaryVectors) {
							...
							SerializeTrialEmbeddedBean.serializeVectorBean(
									vectorBeanEmbedded,
