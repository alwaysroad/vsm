...
public class VSMFeatureVectorsIN {
	...
	public static void main(String... args) throws Exception {
		...
		if (args.length > 2) {
			 ...
			 * features
			 */
			featureDictionarySyn = VSMContant.FEATURE_DICT_IN;
			 ...
			 */
			parsedTreeCorpus = VSMContant.SICK_TRAINING_TREES;
			/*
			 * Necessary to get the appropriate directory structure
			 */
			//treeCountObjLoc = "/afs/inf.ed.ac.uk/group/project/vsm.restored/countmaptest/treecount.ser";
		}
		 ...
		 */
		System.out
				.println("***Getting semantic and syntactic feature dictionaries");
		 ...
		 */
		PennTreeReader treeReader = VSMUtil.getTreeReader(trainingTreesFile
				.getAbsolutePath());
		/*
		...
		while (treeReader.hasNext()) {
			...
			/*
			 * Unmatched parentheses exception. Does this mean that the BLLIP
			 * corpus sometimes does not have correct parse trees? Strange
			 */
			try {
			...
			if (syntaxTree != null) {
				...
				while (nodeTrees.hasNext()) {
					...
					if (!insideTree.isLeaf()
							&& insideTree.getLabel().equalsIgnoreCase("IN")) {
						...
						/*
						 * Setting the object's properties that are stored in
						 * the .ser file
						 */
						...
						/*
						 * Storing the feature vectors in a bean which will be
						...
						/*
						 * Serialize the feature vector bean corresponding to
						 * the particular node. The feature vector bean contains
						 * the sparse inside and outside feature vectors
						 */
		...
		System.out.println("****Done with IN*****");
	}
}
