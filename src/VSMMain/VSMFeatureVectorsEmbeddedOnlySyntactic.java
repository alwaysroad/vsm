...
public class VSMFeatureVectorsEmbeddedOnlySyntactic {
			...
			if (chunks != null) {
				...
				innerloop: for (File chunk : chunks) {
					...
					if (chunk.getName().equalsIgnoreCase(
						 ...
						 */
						// matricesSem =
						// VSMUtil.deserializeCCAVariantsRunSem(chunk
						// .getName());
						...
						int vectorCount = 0;
						/*
						 * The vectors corresponding to that chunk, syntactic
						 * and semantic
						 */
						...
						/*
						 * A very annoying fact about the list files is that it
						 * can sometime return null even if the directory has
						 * files, because maybe too many files are open.
						 */
						if (vectors != null) {
							for (File vector : vectors) {
								...
								if (matricesSyn != null) {
									if (matricesSyn[0] != null) {
										 ...
										 */
										SparseVector phiSyn = vectorBean
												.getPhi();
										// SparseVector phiSem =
										...
										/*
										 * The vectors can be null and hence a
										 * null check, remember we are storing
										 * the null vectors as well
										 */
										if (phiSyn != null) {
											...
											while (sparseVecItr.hasNext()) {
												VectorEntry e = sparseVecItr
														.next();

												/*
												 * Getting the sparse vector
												 * index and values
												 */
												int idx = e.index();
												double val = e.get();
										...
										/*
										 * Serializing the lower dimensional
										 * vector embedding
										 */
										vectorBeanEmbedded
												.setPhi((DenseVector) phiSynEmbedded);
										// vectorBeanEmbedded
										// .setPhiSem((DenseVector)
										// phiSemEmbedded);
									...
									if (matricesSyn[1] != null) {
										 ...
										 * Getting the inside feature vector
										 */
										SparseVector psiSyn = vectorBean
												.getPsi();
										// SparseVector psiSem =
										// vectorBean.getPsiSem();

										/*
										 * Converting the sparse vector into
										 * dense vector
										 */
										...
										if (psiSyn != null) {
											...
											/*
											 * Iterator over the sparse vector
											 * MTJ
											 */
											...
											/*
											 * Multiply the matrix and the
											 * vector, to get the continuous
											 * representation
											 */
											psiSynEmbedded = new DenseVector(
													ZTSyn.numRows());
											ZTSyn.mult(psiDenseSyn,
													psiSynEmbedded);
								...
								/*
								 * Serializing the continuous rep the semantic
								 * and the syntactic
								 */
								...
								/*
								 * Taking the average, to get the average
								...
								/*
								 * Serialize the syntactic embedded feature
								 * vector
								 */
								 ...
								 * similarly form the continuous rep for the
								 * outside feature vectors
								 */
							...
							/*
							 * We need to log this in a file so that we can
							 * handle this later
							 */
							try {
								logWriter = new PrintWriter(
										"/afs/inf.ed.ac.uk/group/project/vsm.restored/log/SICKFeatureVecEmbeddingsLog"
												+ nonTerminal.toUpperCase()
												+ ".txt");
				...
				System.out
						.println("***OOPS chunks are null, the list files has returned null again, Crappy File system****");
				/*
				 * We need to log this in a file so that we can handle this
				 * later
				 */
				try {
					logWriter = new PrintWriter(
							"/afs/inf.ed.ac.uk/group/project/vsm.restored/log/SICKFeatureVecEmbeddingsLog"
									+ nonTerminal.toUpperCase() + ".txt");
					logWriter.println("Directory Path: "
