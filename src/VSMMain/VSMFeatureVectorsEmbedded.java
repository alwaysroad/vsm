...
public class VSMFeatureVectorsEmbedded {
	...
	public static void main(String... args) throws ClassNotFoundException {
		...
		for (File directory : directories) {
			...
			innerloop: for (File chunk : chunks) {
				 ...
				 * feature vector
				 */
				System.out.println("***Getting the projection matrices For***"
						+ chunk.getName());
				/*
				 * The syntactic projection matrices
				 */
				matricesSyn = VSMUtil
						.deserializeCCAVariantsRun(chunk.getName());
				...
				for (File vector : vectors) {
					 ...
					 */
					if (matricesSyn != null && matricesSem != null) {
						if (matricesSyn[0] != null && matricesSem[0] != null) {
							...
							/*
							 * The vectors can be null and hence a null check,
							 * remember we are storing the null vectors as well
							 */
							if (phiSyn != null) {
						...
						if (matricesSyn[1] != null && matricesSem[1] != null) {
							/*
							 * Inside Projection Matrix
							 */
							...
							/*
							 * Converting the sparse vector into dense vector
							 */
					...
					vectorBeanEmbedded.setPsiSem((DenseVector) psiSemEmbedded);
					vectorBeanEmbedded
							.setSyntaxTree(vectorBean.getSyntaxTree());
					vectorBeanEmbedded
							.setInsideTree(vectorBean.getInsideTree());
					...
					// vectorCount));
					/*
					 * Serialize the continuous rep average TODO and similarly
					 * form the continuous rep for the outside feature vectors
					 */
