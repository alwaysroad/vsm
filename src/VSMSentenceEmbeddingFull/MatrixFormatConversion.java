...
public class MatrixFormatConversion {
	...
	public static DenseMatrix createDenseMatrixMTJ(Matrix xomega) {
		DenseMatrix xMTJ = new DenseMatrix(xomega.getRowDimension(),
	...
	public static DenseDoubleMatrix2D createDenseMatrixCOLT(DenseMatrix xmtj) {
		DenseDoubleMatrix2D x_omega = new DenseDoubleMatrix2D(xmtj.numRows(),
				xmtj.numColumns());
	...
	public static DenseDoubleMatrix2D createDenseMatrixCOLT(UpperTriangDenseMatrix xmtj) {
		DenseDoubleMatrix2D x_omega = new DenseDoubleMatrix2D(xmtj.numRows(),
				xmtj.numColumns());
		for (int i = 0; i < xmtj.numRows(); i++) {
			for (int j = 0; j < xmtj.numColumns(); j++) {
	...
	public static FlexCompRowMatrix multLargeSparseMatricesJEIGEN(
			FlexCompRowMatrix x, FlexCompRowMatrix y) {
		SparseMatrixLil prodMatrix = new SparseMatrixLil(x.numRows(),
				y.numColumns());
	...
	public static DenseDoubleMatrix2D multiplySparseDenseScaleRow(
			SparseDoubleMatrix2D x, DenseDoubleMatrix2D y) {

		DenseDoubleMatrix2D prodMatrix = new DenseDoubleMatrix2D(x.rows(),
				y.columns());
		System.out.println("+++Before Multiply+++");
	...
	public static DenseDoubleMatrix2D multiplySparseDenseScaleCol(
			DenseDoubleMatrix2D x, SparseDoubleMatrix2D y) {
	...
	public static jeigen.DenseMatrix createDenseMatrixJEIGEN(
		...
		jeigen.DenseMatrix X = new jeigen.DenseMatrix(uHatT.rows(),
				uHatT.columns());
	...
	public static DenseDoubleMatrix2D createDenseMatrixCOLT(
		...
		DenseDoubleMatrix2D X = new DenseDoubleMatrix2D(uSVNew.rows,
				uSVNew.cols);
